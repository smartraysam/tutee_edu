...
class SmFeesController extends Controller
{
        ...
        try {
            foreach ($request->id as $student) {
                ...
                if ($request->update == 1) {
                    ...
                    $fees_forward->academic_id = YearCheck::getAcademicId();
                    $fees_forward->save();
                }
            }
            DB::commit();
    ...
    public function collectFeesSearch(Request $request)
    {
        ...
        try {
            ...
            $students = $students->where('academic_id', YearCheck::getAcademicId())->where('school_id',Auth::user()->school_id)->where('active_status',1)->get();
    ...
    public function feesGenerateModal(Request $request, $amount, $student_id, $type,$master)
    {
        ...
        try {
            ...
            return view('backEnd.feesCollection.fees_generate_modal', compact('amount','master', 'discounts', 'fees_type_id', 'student_id', 'applied_discount', 'data', 'method'));
        } catch (\Exception $e) {
            Toastr::error('Operation Failed', 'Failed');
            return redirect()->back();
        }
...
// return $request;
            ...
            $result = $fees_payment->save();
                ...
                $fees_assign=SmFeesAssign::where('fees_master_id',$request->master_id)->first();
...
// return  $fees_assigneds;
    ...
    public function bankPaymentSlip(){
        $bank_slips = SmBankPaymentSlip::where('academic_id', YearCheck::getAcademicId())->where('school_id',Auth::user()->school_id)->orderBy('id', 'desc')->get();

        return view('backEnd.feesCollection.bank_payment_slip', compact('bank_slips'));
    }
    ...
    public function approveFeesPayment(Request $request){
        ...
        try {
            ...
            $fees_payment->academic_id = YearCheck::getAcademicId();
            $fees_payment->school_id = Auth::user()->school_id;
            // return $fees_payment;
            $result = $fees_payment->save();
