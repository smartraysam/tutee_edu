...
class SmBookController extends Controller
{
    ...
    public function saveIssueBookData(Request $request)
    {
        ...
        $book_issue = SmBookIssue::where('book_id', $request->book_id)->where('issue_status', '=', 'I')->count();
        $book_available = $book_quantity - $book_issue;
        if ($book_quantity == 0) {
    ...
    public function searchIssuedBook(Request $request)
    {
        ...
        try {
            ...
            if (!empty($request->book_number) &&  $request->book_number != "") {
                $query .= "AND b.book_number = '$request->book_number'";
            }

            // if (!empty($request->subject_id)  &&  $request->book_number != "") {
            //     $query .= "AND b.subject_id = '$request->subject_id'";
            // }

            $query .= "AND i.school_id = " . Auth::user()->school_id;

            // return $query;

            $issueBooks = DB::select(DB::raw("SELECT i.*, b.book_title, b.book_number,
                    b.isbn_no, b.author_name, m.member_type, m.student_staff_id
    ...
    public function store(Request $request)
    {
        ...
        $is_duplicate = LibrarySubject::where('school_id', Auth::user()->school_id)->where('subject_name', $request->subject_name)->first();
        if ($is_duplicate) {
        ...
        $find = LibrarySubject::where('school_id', Auth::user()->school_id)->where('subject_code', $request->subject_code)->first();
        if ($find != "") {
        ...
        try {
            ...
            $subject->academic_id = YearCheck::getAcademicId();
            $result = $subject->save();
    ...
    public function edit(Request $request, $id)
    {
        ...
        try {
            ...
            ->leftjoin('sm_book_categories','sm_book_categories.id','=','library_subjects.sb_category_id')
            ->where('library_subjects.school_id', Auth::user()->school_id)
            ->select('library_subjects.*','sm_book_categories.category_name')->get();$bookCategories = SmBookCategory::where('school_id',Auth::user()->school_id)->get();
            if (ApiBaseMethod::checkUrl($request->fullUrl())) {
                $data = [];
    ...
    public function update(Request $request)
    {
        ...
        $is_duplicate = LibrarySubject::where('school_id', Auth::user()->school_id)->where('id','!=', $request->id)->where('subject_name', $request->subject_name)->first();
        if ($is_duplicate) {
            Toastr::error('Duplicate name found!', 'Failed');
            return redirect()->back()->withErrors($validator)->withInput();
        }
