...
class SmFeesTypeController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $is_duplicate = SmFeesType::where('school_id', Auth::user()->school_id)->where('name', $request->name)->where('fees_group_id', $request->fees_group)->where('academic_id', YearCheck::getAcademicId())->first();
        if ($is_duplicate) {
        ...
        try{
            ...
            $fees_type->academic_id = YearCheck::getAcademicId();
    ...
    public function update(Request $request)
    {
        ...
        $is_duplicate = SmFeesType::where('school_id', Auth::user()->school_id)->where('id','!=', $request->id)->where('name', $request->name)->where('fees_group_id', $request->fees_group)->where('academic_id', YearCheck::getAcademicId())->first();
        if ($is_duplicate) {
            Toastr::error('Duplicate name found!', 'Failed');
            return redirect()->back()->withErrors($validator)->withInput();
        }
