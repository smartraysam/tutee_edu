...
class AcademicController extends Controller
{
    ...
    public function assignSubjectSearch(Request $request)
    {
        ...
        try {
            ...
            $subjects = SmSubject::where('active_status', 1)->where('school_id', Auth::user()->school_id)->where('academic_id', YearCheck::getAcademicId())->get();
    ...
    public function classReportSearch(Request $request)
    {
        ...
        try {
            ...
            $assign_subjects = $assign_subjects->where('academic_id', YearCheck::getAcademicId())->where('school_id', Auth::user()->school_id)->get();
            ...
            foreach ($students as $student) {
                $fees_assigns = SmFeesAssign::where("student_id", $student->id)->where('active_status', 1)->where('academic_id', YearCheck::getAcademicId())->where('school_id', Auth::user()->school_id)->get();
                foreach ($fees_assigns as $fees_assign) {
                    $fees_masters = SmFeesMaster::where('id', $fees_assign->fees_master_id)->where('academic_id', YearCheck::getAcademicId())->where('school_id', Auth::user()->school_id)->get();
