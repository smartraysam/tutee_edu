...
class SmFeesGroupController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $is_duplicate = SmFeesGroup::where('school_id', Auth::user()->school_id)->where('name', $request->name)->where('academic_id', YearCheck::getAcademicId())->first();
        if ($is_duplicate) {
        ...
        try{
            ...
            $visitor->academic_id = YearCheck::getAcademicId();
            $result = $visitor->save();
    ...
    public function update(Request $request)
    {
        ...
        $is_duplicate = SmFeesGroup::where('school_id', Auth::user()->school_id)->where('id','!=', $request->id)->where('name', $request->name)->where('academic_id', YearCheck::getAcademicId())->first();
        if ($is_duplicate) {
            Toastr::error('Duplicate name found!', 'Failed');
            return redirect()->back()->withErrors($validator)->withInput();
        }
