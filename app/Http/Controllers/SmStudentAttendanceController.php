...
class SmStudentAttendanceController extends Controller
{
    ...
    public function studentAttendanceStore(Request $request)
    {
        ...
        try{
            foreach ($request->id as $student) {
                $attendance = SmStudentAttendance::where('student_id', $student)->where('attendance_date', date('Y-m-d', strtotime($request->date)))->where('academic_id', YearCheck::getAcademicId())->where('school_id',Auth::user()->school_id)->first();
    ...
    //                             $import->academic_id = YearCheck::getAcademicId();
    //                             $import->save();
    //                         }
    //                     }
                ...
                foreach ($data as $key => $value) {
                    ...
                    if ($value != "") {
                        if(date('d/m/Y', strtotime($request->attendance_date)) == date('d/m/Y', strtotime($value->attendance_date))){
                            ...
                            if($student != ""){
                                // SmStudentAttendance
                                $attendance_check=SmStudentAttendance::where('student_id',$student->id)->where('attendance_date',date('Y-m-d', strtotime($value->attendance_date)))->first();
                                if ($attendance_check) {
