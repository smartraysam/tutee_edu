...
class SmCommunicateController extends Controller
{
            ...
            if (empty($request->role)) {
                ...
                if ($request->send_through == 'E') {
                    ...
                    foreach ($request->role as $role_id) {
                        ...
                        if ($role_id == 2) {
                            $receiverDetails = SmStudent::select('email', 'full_name', 'mobile')->where('active_status', 1)->where('academic_id', YearCheck::getAcademicId())->get();
                        } elseif ($role_id == 3) {
                            $receiverDetails = SmParent::select('guardians_email as email', 'fathers_name as full_name', 'fathers_mobile as mobile')->where('academic_id', YearCheck::getAcademicId())->get();
                        } else {
                    ...
                    foreach ($request->role as $role_id) {
                        ...
                        if ($role_id == 2) {
                            $receiverDetails = SmStudent::select('email', 'full_name', 'mobile')->where('active_status', 1)->where('academic_id', YearCheck::getAcademicId())->where('school_id', Auth::user()->school_id)->get();
                        } elseif ($role_id == 3) {
                            $receiverDetails = SmParent::select('guardians_email as email', 'fathers_name as full_name', 'fathers_mobile as mobile')->where('school_id', Auth::user()->school_id)->where('academic_id', YearCheck::getAcademicId())->get();
                        } else {
    ...
    public function studStaffByRole(Request $request)
    {
        ...
        try {
            if ($request->id == 2) {
                $allStudents = SmStudent::where('active_status', '=', 1)->where('academic_id', YearCheck::getAcademicId())->where('school_id', Auth::user()->school_id)->get();
                $students = [];
            ...
            if ($request->id == 3) {
                $allParents = SmParent::join('sm_students', 'sm_students.parent_id', '=', 'sm_parents.id')
                    ...
                    ->where('sm_classes.academic_id', YearCheck::getAcademicId())->where('sm_students.school_id', Auth::user()->school_id)->get();
                $parents = [];
    ...
    {
        try {
            $emailSmsLogs = SmEmailSmsLog::where('academic_id', YearCheck::getAcademicId())->orderBy('id', 'DESC')->where('school_id', Auth::user()->school_id)->get();
