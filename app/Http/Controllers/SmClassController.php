...
class SmClassController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        try {
            ...
            $classes = SmClass::where('academic_id', YearCheck::getAcademicId())->where('active_status', '=', 1)->where('school_id', Auth::user()->school_id)->get();
    ...
    public function store(Request $request)
    {
        ...
        $is_duplicate = SmClass::where('school_id', Auth::user()->school_id)->where('class_name', $request->name)->where('academic_id', YearCheck::getAcademicId())->first();
        if ($is_duplicate) {
        ...
        DB::beginTransaction();
            ...
            try {
                ...
                $class->academic_id = YearCheck::getAcademicId();
                $class->save();
                $class->toArray();
                try {
                    ...
                    if ($sections != '') {
                        foreach ($sections as $section) {
                            ...
                            $smClassSection->academic_id = YearCheck::getAcademicId();;
                            $smClassSection->save();
                        }
                    }
                    DB::commit();
            ...
            Toastr::error('Pease select section', 'Failed');
            return redirect()->back();
        }
        if (ApiBaseMethod::checkUrl($request->fullUrl())) {
    ...
    public function edit(Request $request, $id)
    {
        ...
        try {
            ...
            $classes = SmClass::where('active_status', '=', 1)->orderBy('id', 'desc')->where('academic_id', YearCheck::getAcademicId())->where('school_id', Auth::user()->school_id)->get();
    ...
    public function update(Request $request)
    {
        ...
        $is_duplicate = SmClass::where('school_id', Auth::user()->school_id)->where('id','!=', $request->id)->where('class_name', $request->name)->where('academic_id', YearCheck::getAcademicId())->first();
        if ($is_duplicate) {
            Toastr::error('Duplicate name found!', 'Failed');
            return redirect()->back()->withErrors($validator)->withInput();
        }
