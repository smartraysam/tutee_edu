...
class SmApiController extends Controller
{
            ...
            foreach ($lastest_data as $device_log) {
                ...
                if ($user_table) {
                    if ($user_table->role_id == 2) {
                        ...
                        /* $to_name    = $sm_students->full_name;
                        $to_email   = $sm_students->email;
                        $email_sms_title = 'Student attendance';
                        $data = array('name' => $to_name, 'email_sms_title' => $email_sms_title, 'description' => 'Please see your attendance'); */
                        if ($sm_students) {
                        ...
                        $attendance = SmStudentAttendance::where('student_id', $sm_students->id)->where('attendance_date', date('Y-m-d', strtotime($device_log->checktime)))->where('academic_id', YearCheck::getAcademicId())->first();
                        if ($attendance == "") {
                        ...
                        if ($attendance_start_time >= $d_start_time) {
                            ...
                            // if($attendance_setting->start_time_sms == 1 && isset($sm_students->id) && isset($sms_template->attendance_sms)){
                            //     $sms = SMSBody($sms_template->attendance_sms, $sm_students->id,date('g:i a', strtotime($device_log->checktime)));
                            //     $t = sendSMSBio($to_mobile, $sms);
                            // }
                            ...
                            //      $sms = $this->SMSBody($sms_template->late_sms, $sm_students->id,date('g:i a', strtotime($device_log->checktime)));
                            //     $this->sendSMSBio($to_mobile, $sms);
                            // }
                            ...
                            //     /* $this->sendEmailBio($data, $to_name, $to_email, $email_sms_title); */
                            //      $sms = SMSBody($sms_template->absent, $sm_students->id,date('g:i a', strtotime($device_log->checktime)));
                            //     $this->sendSMSBio($to_mobile, $sms);
                            // }
                            ...
                            //     /* $this->sendEmailBio($data, $to_name, $to_email, $email_sms_title); */
                            //      $sms = SMSBody($sms_template->er_checkout, $sm_students->id,date('g:i a', strtotime($device_log->checktime)));
                            //     $this->sendSMSBio($to_mobile, $sms);
                            // }
                            ...
                            //     /* $this->sendEmailBio($data, $to_name, $to_email, $email_sms_title); */
                            //      $sms = SMSBody($sms_template->st_checkout, $sm_students->id,date('g:i a', strtotime($device_log->checktime)));
                            //     $this->sendSMSBio($to_mobile, $sms);
                            // }
                        ...
                        $attendance->academic_id = YearCheck::getAcademicId();
                        $attendance->save();
                    } else {
                        $sm_staff = SmStaff::where('user_id', $device_log->userid)->first();
                        /* $to_name    = $sm_staff->full_name;
                        $to_email   = $sm_staff->email;
                        $email_sms_title = 'Staff attendance';
                        $data = array('name' => $to_name, 'email_sms_title' => $email_sms_title, 'description' => 'Please see your attendance'); */
                        if ($sm_staff) {
    ...
    public function storeRoom(Request $request)
    {
        ...
        $room_list->academic_id = YearCheck::getAcademicId();
    ...
    public function updateRoom(Request $request)
    {
        ...
        $room->academic_id = YearCheck::getAcademicId();
        $result = $room->save();
    ...
    public function addDormitory(Request $request)
    {
        ...
        $dormitory_list->academic_id = YearCheck::getAcademicId();
    ...
    public function saas_deleteDormitory(Request $request,$school_id, $id)
    {
        $tables = \App\tableList::getTableList('dormitory_id',$id);
        try {
            // $dormitory_list = SmDormitoryList::where('school_id',$school_id)->delete($id);
    ...
    public function addBook(Request $request)
    {
        ...
        $books->academic_id = YearCheck::getAcademicId();
        $results = $books->save();
    ...
    public function library_member_store(Request $request)
    {
        ...
        if (!empty($isExist_staff_id)) {
            ...
            $members->academic_id = YearCheck::getAcademicId();
            // return $members;
                    ...
                    foreach ($sections as $section) {
                        ...
                        foreach ($request->subjects_ids as $subject_id) {
                            ...
                            if (in_array($subject_id, $eligible_subjects)) {
                                ...
                                $exam->academic_id = YearCheck::getAcademicId();
                                // return $exam;
                                $exam->save();
                                ...
                                for ($i = 0; $i < $length; $i++) {
                                    ...
                                    // return $newSetupExam;
                                    $newSetupExam->academic_id = YearCheck::getAcademicId();
    ...
    public function NewExamSchedule(Request $request)
    {
        ...
        if ($request->assigned_id == "") {
            $check_date = SmExamSchedule::where('class_id', $request->class_id)->where('section_id', $request->section_id)->where('exam_term_id', $request->exam_term_id)->where('date', date('Y-m-d', strtotime($request->date)))->where('exam_period_id', $request->exam_period_id)->where('academic_id', YearCheck::getAcademicId())->get();
        } else {
            $check_date = SmExamSchedule::where('id', '!=', $request->assigned_id)->where('class_id', $request->class_id)->where('section_id', $request->section_id)->where('exam_term_id', $request->exam_term_id)->where('date', date('Y-m-d', strtotime($request->date)))->where('exam_period_id', $request->exam_period_id)->where('academic_id', YearCheck::getAcademicId())->get();
        }
    ...
    public function saas_NewExamSchedule(Request $request, $school_id)
    {
        ...
        if ($request->assigned_id == "") {
            $check_date = SmExamSchedule::where('class_id', $request->class_id)->where('section_id', $request->section_id)->where('exam_term_id', $request->exam_term_id)->where('date', date('Y-m-d', strtotime($request->date)))->where('exam_period_id', $request->exam_period_id)->where('academic_id', YearCheck::getAcademicId())->where('school_id',$school_id)->get();
        } else {
            $check_date = SmExamSchedule::where('id', '!=', $request->assigned_id)->where('class_id', $request->class_id)->where('section_id', $request->section_id)->where('exam_term_id', $request->exam_term_id)->where('date', date('Y-m-d', strtotime($request->date)))->where('exam_period_id', $request->exam_period_id)->where('academic_id', YearCheck::getAcademicId())->where('school_id',$school_id)->get();
        }
                ...
                if (Hash::check($request->password, $user->password)) {
                    ...
                    if ($role_id == 2) {
                        ...
                        $data['userDetails'] = DB::table('sm_students')->select('sm_students.*', 'sm_parents.*', 'sm_classes.*', 'sm_sections.*')
                            ...
                            ->where('sm_students.user_id', $user->id)
                            ->first();
                        $data['userData']= StudentRegistration::whereEmail($request->email)->first();
                        $data['religion'] = DB::table('sm_students')->select('sm_base_setups.base_setup_name as name')
                            ->join('sm_base_setups', 'sm_base_setups.id', '=', 'sm_students.religion_id')
                            ->where('sm_students.user_id', $user->id)
                            ->first();
    ...
    public function postal_dispatch_show(Request $request, $id)
    {
        ...
        try {
            $postal_dispatchs = SmPostalDispatch::where('academic_id', YearCheck::getAcademicId())->get();
    ...
    public function saas_postal_dispatch_show(Request $request,$school_id, $id)
    {
        ...
        try {
            $postal_dispatchs = SmPostalDispatch::where('academic_id', YearCheck::getAcademicId())->where('school_id',$school_id)->get();
    ...
    {
        try {
            $classes = SmClass::where('active_status', 1)->where('academic_id', YearCheck::getAcademicId())->where('school_id',$school_id)->get();
            $student_list = DB::table('sm_students')
                ->join('sm_classes', 'sm_students.class_id', '=', 'sm_classes.id')
                ->join('sm_sections', 'sm_students.section_id', '=', 'sm_sections.id')
                ->where('sm_students.academic_id', YearCheck::getAcademicId())
    ...
    public function studentView(Request $request, $id)
    {
        try {
            ...
            // return $vehicle;
            $fees_assigneds = SmFeesAssign::where('student_id', $id)->where('academic_id', YearCheck::getAcademicId())->get();
            //  return $fees_assigneds;
            $fees_discounts = SmFeesAssignDiscount::where('student_id', $id)->where('academic_id', YearCheck::getAcademicId())->get();
            // $documents = SmStudentDocument::where('student_staff_id', $id)->where('type', 'stu')->where('academic_id', YearCheck::getAcademicId())->get();
            $documents = SmStudentDocument::where('student_staff_id', $id)->where('academic_id', YearCheck::getAcademicId())->get();

            $timelines = SmStudentTimeline::where('staff_student_id', $id)->where('type', 'stu')->where('academic_id', YearCheck::getAcademicId())->get();
    ...
    public function saas_studentView(Request $request,$school_id, $id)
    {
        try {
            ...
            // return $vehicle;
            $fees_assigneds = SmFeesAssign::where('student_id', $id)->where('academic_id', YearCheck::getAcademicId())->where('school_id',$school_id)->get();
            //  return $fees_assigneds;
            $fees_discounts = SmFeesAssignDiscount::where('student_id', $id)->where('academic_id', YearCheck::getAcademicId())->where('school_id',$school_id)->get();
            // $documents = SmStudentDocument::where('student_staff_id', $id)->where('type', 'stu')->where('academic_id', YearCheck::getAcademicId())->get();
            $documents = SmStudentDocument::where('student_staff_id', $id)->where('academic_id', YearCheck::getAcademicId())->where('school_id',$school_id)->get();

            $timelines = SmStudentTimeline::where('staff_student_id', $id)->where('type', 'stu')->where('academic_id', YearCheck::getAcademicId())->where('school_id',$school_id)->get();
    ...
    public function saas_studentEdit(Request $request,$school_id, $id)
    {
        try {
            ...
            $classes = SmClass::where('active_status', '=', '1')->where('academic_id', YearCheck::getAcademicId())->where('school_id',$school_id)->get();
            $sections = SmSection::where('active_status', '=', '1')->where('academic_id', YearCheck::getAcademicId())->where('school_id',$school_id)->get();
    ...
    public function studentSearch(Request $request)
    {
        ...
        try {
            ...
            $students = SmStudent::where('class_id', $request->class)->where('section_id', $request->section)->where('active_status', 1)->where('academic_id', YearCheck::getAcademicId())->get();
            ...
            $attendance_type = "";
            foreach ($students as $student) {
                $attendance = SmStudentAttendance::where('student_id', $student->id)->where('attendance_date', date('Y-m-d', strtotime($request->attendance_date)))->where('academic_id', YearCheck::getAcademicId())->first();
    ...
    public function studentAttendanceStore(Request $request)
    {
        ...
        try {
            foreach ($request->id as $student) {
                $attendance = SmStudentAttendance::where('student_id', $student)->where('attendance_date', date('Y-m-d', strtotime($request->date)))->where('academic_id', YearCheck::getAcademicId())->first();
    ...
    public function studentAttendanceReport(Request $request)
    {
        try {
            ...
            $genders = SmBaseSetup::where('active_status', '=', '1')->where('base_group_id', '=', '1')->where('academic_id', YearCheck::getAcademicId())->get();
    ...
    public function saas_studentAttendanceReport(Request $request,$school_id)
    {
        try {
            ...
            $genders = SmBaseSetup::where('active_status', '=', '1')->where('base_group_id', '=', '1')->where('academic_id', YearCheck::getAcademicId())->where('school_id',$school_id)->get();
    ...
    public function studentAttendanceReportSearch(Request $request)
    {
        ...
        try {
            ...
            $attendances = [];
            foreach ($students as $student) {
                $attendance = SmStudentAttendance::where('student_id', $student->id)->where('attendance_date', 'like', $request->year . '-' . $request->month . '%')->where('academic_id', YearCheck::getAcademicId())->get();
    ...
    public function saas_studentAttendanceReportSearch(Request $request, $school_id)
    {
        ...
        try {
            ...
            $days = cal_days_in_month(CAL_GREGORIAN, $request->month, $request->year);
            $classes = SmClass::where('active_status', 1)->where('academic_id', YearCheck::getAcademicId())->where('school_id',$school_id)->get();
            $students = SmStudent::where('class_id', $request->class)->where('section_id', $request->section)->where('active_status', 1)->where('academic_id', YearCheck::getAcademicId())->where('school_id',$school_id)->get();

            $attendances = [];
            foreach ($students as $student) {
                $attendance = SmStudentAttendance::where('student_id', $student->id)->where('attendance_date', 'like', $request->year . '-' . $request->month . '%')->where('academic_id', YearCheck::getAcademicId())->where('school_id',$school_id)->get();
                if (count($attendance) != 0) {
                    $attendances[] = $attendance;
                }
            }
    ...
    public function student_type_index(Request $request)
    {
        ...
        try {
            // $student_types = SmStudentCategory::where('academic_id', YearCheck::getAcademicId())->get();
    ...
    public function student_group_index(Request $request)
    {
        ...
        try {
            $student_groups = SmStudentGroup::where('academic_id', YearCheck::getAcademicId())->get();
    ...
    public function saas_student_group_index(Request $request, $school_id)
    {
        ...
        try {
            $student_groups = SmStudentGroup::where('academic_id', YearCheck::getAcademicId())->where('school_id',$school_id)->get();
    ...
    public function studentPromote_index(Request $request)
    {
        try {
            ...
            $exams = SmExamType::where('active_status', 1)->where('academic_id', YearCheck::getAcademicId())->get();
    ...
    public function studentCurrentSearch(Request $request)
    {
        ...
        try {
            if ($request->result == 'P') {
                ...
                $students = SmStudent::where('class_id', '=', $request->current_class)->where('session_id', '=', $request->current_session)->where('section_id', $request->section)->where('active_status', 1)->where('academic_id', YearCheck::getAcademicId())->get();
            }
            $current_session = $request->current_session;
            $current_class = $request->current_class;
            $sessions = SmAcademicYear::where('active_status', 1)->where('academic_id', YearCheck::getAcademicId())->get();
        ...
        try {
            if ($request->result == 'P') {
                ...
                if (@$students == 0) {
                    ...
                    $students['students'] = [];
                foreach ($students['allresult_data'] as $key => $value) {
                    $d = SmStudent::where('id', $value->student_id)->where('academic_id', YearCheck::getAcademicId())->where('school_id',$school_id)-first();
                ...
                $students = SmStudent::where('class_id', '=', $request->current_class)->where('session_id', '=', $request->current_session)->where('section_id', $request->section)->where('active_status', 1)->where('academic_id', YearCheck::getAcademicId())->where('school_id',$school_id)->get();
            }
            $current_session = $request->current_session;
            $current_class = $request->current_class;
            $sessions = SmAcademicYear::where('active_status', 1)->where('academic_id', YearCheck::getAcademicId())->where('school_id',$school_id)-get();
    ...
    public function studentPromoteStore(Request $request)
    {
        ...
        try {
            ...
            if ($request->promote_class == "" || $request->promote_session == "") {
                $students = SmStudent::where('class_id', '=', $request->promote_class)->where('session_id', '=', $request->promote_session)->where('academic_id', YearCheck::getAcademicId())->get();
                //dd($students);
                ...
                try {
                    ...
                    DB::rollback();
                    $students = SmStudent::where('class_id', '=', $request->current_class)->where('session_id', '=', $request->current_session)->where('academic_id', YearCheck::getAcademicId())->get();
    ...
    {
        try {
            $students = SmStudent::where('active_status', 0)->where('academic_id', YearCheck::getAcademicId())->get();
            $classes = SmClass::where('active_status', 1)->where('academic_id', YearCheck::getAcademicId())->get();
    ...
    {
        try {
            $students = SmStudent::where('active_status', 0)->where('academic_id', YearCheck::getAcademicId())->where('school_id',$school_id)->get();
            $classes = SmClass::where('active_status', 1)->where('academic_id', YearCheck::getAcademicId())->where('school_id',$school_id)->get();
    ...
    public function disabledStudentSearch(Request $request)
    {
        try {
            ...
            $students = $students->where('academic_id', YearCheck::getAcademicId())->get();
    ...
    public function saas_disabledStudentSearch(Request $request, $school_id)
    {
        try {
            $students = SmStudent::query();
            $students->where('academic_id', YearCheck::getAcademicId())->where('active_status', 0)->where('school_id',$school_id);
            if ($request->class != "") {
                $students->where('class_id', $request->class);
            }
            if ($request->section != "") {
            ...
            $students = $students->where('academic_id', YearCheck::getAcademicId())->where('school_id',$school_id)->get();
    ...
    public function uploadContentList(Request $request)
    {
        ...
        try {
            $contentTypes = SmContentType::where('academic_id', YearCheck::getAcademicId())->get();

            if (Auth()->user()->id == 1 || Auth()->user()->id == 5) {
                $uploadContents = SmTeacherUploadContent::where('available_for_admin', 1)->orWhere('created_by', Auth::user()->id)->where('academic_id', YearCheck::getAcademicId())->get();
            } else {
                $uploadContents = SmTeacherUploadContent::Where('created_by', Auth::user()->id)->where('academic_id', YearCheck::getAcademicId())->get();
            }
    ...
    public function saas_uploadContentList(Request $request, $school_id)
    {
        ...
        try {
            $contentTypes = SmContentType::where('academic_id', YearCheck::getAcademicId())->where('school_id',$school_id)->get();

            if (Auth()->user()->id == 1 || Auth()->user()->id == 5) {
                $uploadContents = SmTeacherUploadContent::where('available_for_admin', 1)->orWhere('created_by', Auth::user()->id)->where('academic_id', YearCheck::getAcademicId())->where('school_id',$school_id)->get();
            } else {
                $uploadContents = SmTeacherUploadContent::Where('created_by', Auth::user()->id)->where('academic_id', YearCheck::getAcademicId())->where('school_id',$school_id)->get();
            }
    ...
    public function saveUploadContent(Request $request)
    {
        ...
        try {
            ...
            foreach ($request->available_for as $value) {
                if ($value == 'admin') {
                    $roles = InfixRole::where('id', '=', 1) /* ->where('id', '!=', 2)->where('id', '!=', 3)->where('id', '!=', 9) */->where('academic_id', YearCheck::getAcademicId())->where(function ($q) {
                $q->where('school_id', Auth::user()->school_id)->orWhere('type', 'System');
            })->get();
                    foreach ($roles as $role) {
                        $staffs = SmStaff::where('role_id', $role->id)->where('academic_id', YearCheck::getAcademicId())->get();
                ...
                if ($value == 'student') {
                    if (isset($request->all_classes)) {
                        $students = SmStudent::select('id', 'user_id')->where('academic_id', YearCheck::getAcademicId())->get();
                        foreach ($students as $student) {
                            $notification = new SmNotification;
                        ...
                        $students = SmStudent::select('id')->where('class_id', $request->class)->where('section_id', $request->section)->where('academic_id', YearCheck::getAcademicId())->get();
                        foreach ($students as $student) {
                            $notification = new SmNotification;
                            $notification->user_id = $student->id;
                            $notification->role_id = 2;
    ...
    public function assignmentList(Request $request)
    {
        ...
        try {
            ...
            if (Auth()->user()->id == 1) {
                $uploadContents = SmTeacherUploadContent::where('content_type', 'as')->where('academic_id', YearCheck::getAcademicId())->get();
            } else {
                $uploadContents = SmTeacherUploadContent::where('content_type', 'as')->Where('created_by', Auth::user()->id)->where('academic_id', YearCheck::getAcademicId())->get();
            }
    ...
    public function studyMetarialList(Request $request)
    {
        ...
        try {
            if (Auth()->user()->id == 1) {
                $uploadContents = SmTeacherUploadContent::where('content_type', 'st')->where('academic_id', YearCheck::getAcademicId())->get();
            } else {
                $uploadContents = SmTeacherUploadContent::where('content_type', 'st')->Where('created_by', Auth::user()->id)->where('academic_id', YearCheck::getAcademicId())->get();
            }
    ...
    public function saas_studyMetarialList(Request $request , $school_id)
    {
        ...
        try {
            if (Auth()->user()->id == 1) {
                $uploadContents = SmTeacherUploadContent::where('content_type', 'st')->where('academic_id', YearCheck::getAcademicId())->where('school_id',$school_id)->get();
            } else {
                $uploadContents = SmTeacherUploadContent::where('content_type', 'st')->Where('created_by', Auth::user()->id)->where('academic_id', YearCheck::getAcademicId())->where('school_id',$school_id)->get();
            }
    ...
    public function syllabusList(Request $request)
    {
        try {
            if (Auth()->user()->id == 1) {
                $uploadContents = SmTeacherUploadContent::where('content_type', 'sy')->where('academic_id', YearCheck::getAcademicId())->get();
            } else {
                $uploadContents = SmTeacherUploadContent::where('content_type', 'sy')->Where('created_by', Auth::user()->id)->where('academic_id', YearCheck::getAcademicId())->get();
            }
    ...
    public function saas_syllabusList(Request $request, $school_id)
    {
        try {
            if (Auth()->user()->id == 1) {
                $uploadContents = SmTeacherUploadContent::where('content_type', 'sy')->where('academic_id', YearCheck::getAcademicId())->where('school_id',$school_id)->get();
            } else {
                $uploadContents = SmTeacherUploadContent::where('content_type', 'sy')->Where('created_by', Auth::user()->id)->where('academic_id', YearCheck::getAcademicId())->where('school_id',$school_id)->get();
            }
    ...
    public function otherDownloadList(Request $request)
    {
        ...
        try {
            if (Auth()->user()->id == 1) {
                $uploadContents = SmTeacherUploadContent::where('content_type', 'ot')->where('academic_id', YearCheck::getAcademicId())->get();
            } else {
                $uploadContents = SmTeacherUploadContent::where('content_type', 'ot')->Where('created_by', Auth::user()->id)->where('academic_id', YearCheck::getAcademicId())->get();
            }
    ...
    public function saas_otherDownloadList(Request $request, $school_id)
    {
        ...
        try {
            if (Auth()->user()->id == 1) {
                $uploadContents = SmTeacherUploadContent::where('content_type', 'ot')->where('academic_id', YearCheck::getAcademicId())->where('school_id',$school_id)->get();
            } else {
                $uploadContents = SmTeacherUploadContent::where('content_type', 'ot')->Where('created_by', Auth::user()->id)->where('academic_id', YearCheck::getAcademicId())->where('school_id',$school_id)->get();
            }
    ...
    public function feesDiscountAssignSearch(Request $request)
    {
        ...
        try {
            ...
            // $fees_assign_groups = SmFeesMaster::where('fees_group_id', $request->fees_group_id)->where('academic_id', YearCheck::getAcademicId())->get();
    ...
    public function staffAttendanceSearch(Request $request)
    {
        ...
        try {
            $date = $request->attendance_date;
            // $roles = InfixRole::where('id', '!=', 3)->where('id', '!=', 2)->get();
            $roles = InfixRole::where('active_status', '=', '1')->where('id', '!=', 1)->where('id', '!=', 2)->where('id', '!=', 3)->where('id', '!=', 10)->where(function ($q) {
    ...
    public function classRoutineSearch(Request $request)
    {
        ...
        try {
            ...
            $sm_weekends = SmWeekend::where('academic_id', YearCheck::getAcademicId())->where('school_id', Auth::user()->school_id)->orderBy('order', 'ASC')->where('active_status', 1)->get();
            $classes = SmClass::where('active_status', 1)->where('academic_id', YearCheck::getAcademicId())->get();
    ...
    public function subject_edit(Request $request, $id)
    {
        ...
        try {
            ...
            $subjects = SmSubject::where('active_status', 1)->orderBy('id', 'DESC')->where('academic_id', YearCheck::getAcademicId())->get();
    ...
    public function saas_subject_edit(Request $request,$school_id, $id)
    {
        ...
        try {
            ...
            $subjects = SmSubject::where('active_status', 1)->orderBy('id', 'DESC')->where('academic_id', YearCheck::getAcademicId())->where('school_id',$school_id)->get();
    ...
    public function class_index(Request $request)
    {
        try {
            $sections = SmSection::where('active_status', '=', 1)->where('academic_id', YearCheck::getAcademicId())->get();
            $classes = SmClass::where('academic_id', YearCheck::getAcademicId())->where('active_status', '=', 1)->where('academic_id', YearCheck::getAcademicId())->get();
    ...
    public function saas_class_index(Request $request, $school_id)
    {
        try {
            $sections = SmSection::where('active_status', '=', 1)->where('academic_id', YearCheck::getAcademicId())->where('school_id',$school_id)->get();
            $classes = SmClass::where('academic_id', YearCheck::getAcademicId())->where('active_status', '=', 1)->where('academic_id', YearCheck::getAcademicId())->where('school_id',$school_id)->get();
    ...
    public function class_edit(Request $request, $id)
    {
        ...
        try {
            ...
            $sectionByNames = SmClassSection::select('section_id')->where('class_id', '=', $classById->id)->where('academic_id', YearCheck::getAcademicId())->get();
            ...
            $classes = SmClass::where('active_status', '=', 1)->orderBy('id', 'desc')->where('academic_id', YearCheck::getAcademicId())->get();
    ...
    public function saas_class_edit(Request $request,$school_id, $id)
    {
        ...
        try {
            ...
            $sectionByNames = SmClassSection::select('section_id')->where('class_id', '=', $classById->id)->where('academic_id', YearCheck::getAcademicId())->where('school_id',$school_id)->get();
            ...
            $classes = SmClass::where('active_status', '=', 1)->orderBy('id', 'desc')->where('academic_id', YearCheck::getAcademicId())->where('school_id',$school_id)->get();
    ...
    public function Section_edit(Request $request, $id)
    {
        ...
        try {
            ...
            $sections = SmSection::where('active_status', '=', 1)->orderBy('id', 'desc')->where('academic_id', YearCheck::getAcademicId())->get();
    ...
    public function saas_Section_edit(Request $request,$school_id, $id)
    {
        ...
        try {
            ...
            $sections = SmSection::where('active_status', '=', 1)->orderBy('id', 'desc')->where('academic_id', YearCheck::getAcademicId())->where('school_id',$school_id)->get();
    ...
    public function class_Routine(Request $request, $id = null)
    {
        ...
        $sm_weekends = SmWeekend::where('academic_id', YearCheck::getAcademicId())->where('school_id', Auth::user()->school_id)->orderBy('order', 'ASC')->where('active_status', 1)->get();
        $class_times = SmClassTime::where('type', 'class')->where('academic_id', YearCheck::getAcademicId())->get();
        ...
        if (ApiBaseMethod::checkUrl($request->fullUrl())) {
            ...
            $weekenD = SmWeekend::where('academic_id', YearCheck::getAcademicId())->where('school_id', Auth::user()->school_id)->get();
            foreach ($weekenD as $row) {
                $data[$row->name] = DB::table('sm_class_routine_updates')
                    ...
                    ])->where('sm_class_routine_updates.academic_id', YearCheck::getAcademicId())->get();
            }
        ...
        public function saas_class_Routine(Request $request,$school_id, $id = null)
    {
        ...
        $sm_weekends = SmWeekend::orderBy('order', 'ASC')->where('active_status', 1)->where('academic_id', YearCheck::getAcademicId())->where('school_id', Auth::user()->school_id)->get();
        $class_times = SmClassTime::where('type', 'class')->where('academic_id', YearCheck::getAcademicId())->where('school_id',$school_id)->get();
        ...
        if (ApiBaseMethod::checkUrl($request->fullUrl())) {
            ...
            foreach ($weekenD as $row) {
                $data[$row->name] = DB::table('sm_class_routine_updates')
                    ...
                    ->where([
                        ['sm_class_routine_updates.class_id', $class_id], ['sm_class_routine_updates.section_id', $section_id], ['sm_class_routine_updates.day', $row->id],
                    ])->where('sm_class_routine_updates.academic_id', YearCheck::getAcademicId())
    ...
    public function homeworkList(Request $request)
    {
        ...
        try {
            set_time_limit(900);
            $homeworkLists = SmHomework::where('academic_id', YearCheck::getAcademicId())->get();
            $classes = SmClass::where('active_status', '=', '1')->where('academic_id', YearCheck::getAcademicId())->get();
    ...
    public function saas_homeworkList(Request $request,$school_id)
    {
        ...
        try {
            set_time_limit(900);
            $homeworkLists = SmHomework::where('academic_id', YearCheck::getAcademicId())->where('school_id',$school_id)->get();
            $classes = SmClass::where('active_status', '=', '1')->where('academic_id', YearCheck::getAcademicId())->where('school_id',$school_id)->get();
    ...
    public function searchHomework(Request $request)
    {
        ...
        try {
            ...
            $homeworkLists = $homeworkLists->where('academic_id', YearCheck::getAcademicId())->get();
    ...
    public function saas_searchHomework(Request $request, $school_id)
    {
        ...
        try {
            ...
            $homeworkLists = $homeworkLists->where('academic_id', YearCheck::getAcademicId())->where('school_id',$school_id)->get();
    ...
    public function sendEmailSmsView(Request $request)
    {
        try {
            ...
            $classes = SmClass::where('academic_id', YearCheck::getAcademicId())->get();
    ...
    public function saas_sendEmailSmsView(Request $request, $school_id)
    {
        try {
            ...
            $classes = SmClass::where('academic_id', YearCheck::getAcademicId())->where('school_id',$school_id)->get();
    ...
    {
        try {
            $classes = SmClass::where('active_status', 1)->where('academic_id', YearCheck::getAcademicId())
                ->get();
            $dormitories = SmDormitoryList::where('active_status', 1)->get();
            $students = SmStudent::where('active_status', 1)->where('academic_id', YearCheck::getAcademicId())
    ...
    public function studentDormitoryReportSearch(Request $request)
    {
        ...
        try {
            ...
            $students = $students->where('academic_id', YearCheck::getAcademicId())
                ->get();
    ...
    {
        try {
            $students = SmStudent::where('active_status', 1)->where('academic_id', YearCheck::getAcademicId())->get();
            $classes = SmClass::where('active_status', 1)->where('academic_id', YearCheck::getAcademicId())->get();
    ...
    public function studentHistory(Request $request)
    {
        try {
            $classes = SmClass::where('active_status', 1)->where('academic_id', YearCheck::getAcademicId())->get();
            $students = SmStudent::where('active_status', 1)->where('academic_id', YearCheck::getAcademicId())->get();
            $admission_years = SmStudent::groupBy('admission_date')->select('admission_date')->where('academic_id', YearCheck::getAcademicId())->get();
    ...
    public function classReportSearch(Request $request)
    {
        ...
        try {
            $class = SmClass::where('academic_id', YearCheck::getAcademicId())->where('id', $request->class)->first();
            if ($request->section != "") {
                $section = SmSection::where('academic_id', YearCheck::getAcademicId())->where('id', $request->section)->first();
            } else {
                $section = '';
            }
            ...
            $assign_subjects->where('academic_id', YearCheck::getAcademicId())->where('active_status', 1);
            if ($request->section != "") {
            ...
            $assign_class_teacher->where('academic_id', YearCheck::getAcademicId())->where('active_status', 1);
            if ($request->section != "") {
            ...
            $total_assign = 0;
            foreach ($students as $student) {
                $fees_assigns = SmFeesAssign::where('academic_id', YearCheck::getAcademicId())->where("student_id", $student->id)->where('active_status', 1)->get();
                foreach ($fees_assigns as $fees_assign) {
                    $fees_masters = SmFeesMaster::where('academic_id', YearCheck::getAcademicId())->where('id', $fees_assign->fees_master_id)->get();
                    foreach ($fees_masters as $fees_master) {
                        $total_collection = $total_collection + SmFeesPayment::where('academic_id', YearCheck::getAcademicId())->where('student_id', $student->id)->where('fees_type_id', $fees_master->fees_type_id)->sum('amount');
                    }
                }

                foreach ($fees_assigns as $fees_assign) {
                    $fees_master = SmFeesMaster::where('academic_id', YearCheck::getAcademicId())->where('id', $fees_assign->fees_master_id)->first();
    ...
    public function examRoutineReportSearch(Request $request)
    {
        ...
        try {
            $exam_types = SmExamType::where('academic_id', YearCheck::getAcademicId())->get();
            $exam_periods = SmClassTime::where('type', 'exam')->where('academic_id', YearCheck::getAcademicId())->get();
            $exam_routines = SmExamSchedule::where('exam_term_id', $request->exam)->orderBy('date', 'ASC')->where('academic_id', YearCheck::getAcademicId())->get();
    ...
    public function meritListReportSearch(Request $request)
    {
        try {
            ...
            if ($request->method() == 'POST') {
                ...
                $subjects = SmSubject::where('active_status', 1)->where('academic_id', YearCheck::getAcademicId())->get();
                $assign_subjects = SmAssignSubject::where('class_id', $class->id)->where('section_id', $section->id)->where('academic_id', YearCheck::getAcademicId())->get();
                $class_name = $class->class_name;


                $exam_name = $exam->title;

                $eligible_subjects       = SmAssignSubject::where('class_id', $InputClassId)->where('section_id', $InputSectionId)->where('academic_id', YearCheck::getAcademicId())->get();
                $eligible_students       = SmStudent::where('class_id', $InputClassId)->where('section_id', $InputSectionId)->where('academic_id', YearCheck::getAcademicId())->get();
                ...
                $allresult_data = SmTemporaryMeritlist::where('iid', $iid)->orderBy('gpa_point', 'desc')->where('academic_id', YearCheck::getAcademicId())->get();
                $merit_serial = 1;
                ...
                // $optional_subject_setup=SmClassOptionalSubject::

                $allresult_data = SmTemporaryMeritlist::orderBy('merit_order', 'asc')->where('exam_id', '=', $InputClassId)->where('academic_id', YearCheck::getAcademicId())->get();
    ...
    public function onlineExamReport(Request $request)
    {
        ...
        try {
            $exams = SmOnlineExam::where('active_status', 1)->where('academic_id', YearCheck::getAcademicId())->get();
            $classes = SmClass::where('active_status', 1)->where('academic_id', YearCheck::getAcademicId())->get();
    ...
    public function onlineExamReportSearch(Request $request)
    {
        ...
        try {
            ...
            $students = SmStudent::where('class_id', $request->class)->where('section_id', $request->section)->where('academic_id', YearCheck::getAcademicId())->get();
    ...
    public function markSheetReportStudentSearch(Request $request)
    {
        ...
        try {
            $exams = SmExamType::where('active_status', 1)->where('academic_id', YearCheck::getAcademicId())->get();
            $classes        =   SmClass::where('active_status', 1)->where('academic_id', YearCheck::getAcademicId())->get();
            $exam_types     =   SmExamType::where('active_status', 1)->where('academic_id', YearCheck::getAcademicId())->get();

            $subjects = SmAssignSubject::where([['class_id', $request->class], ['section_id', $request->section]])->where('academic_id', YearCheck::getAcademicId())->get();
            ...
            $is_result_available = SmResultStore::where([['class_id', $request->class], ['exam_type_id', $request->exam], ['section_id', $request->section], ['student_id', $request->student]])->where('academic_id', YearCheck::getAcademicId())->get();
            ...
            $subjects = SmAssignSubject::where('class_id', $request->class)->where('section_id', $request->section)->where('academic_id', YearCheck::getAcademicId())->get();
            $exams = SmExamType::where('active_status', 1)->where('academic_id', YearCheck::getAcademicId())->get();
            $classes = SmClass::where('active_status', 1)->where('academic_id', YearCheck::getAcademicId())->get();
            $grades = SmMarksGrade::where('active_status', 1)->where('academic_id', YearCheck::getAcademicId())->get();
    ...
    public function tabulationSheetReportSearch(Request $request)
    {
        ...
        try {
            ...
            if ($request->student == "") {
                $eligible_subjects       = SmAssignSubject::where('class_id', $class_id)->where('section_id', $section_id)->where('academic_id', YearCheck::getAcademicId())->get();
                $eligible_students       = SmStudent::where('class_id', $class_id)->where('section_id', $section_id)->where('academic_id', YearCheck::getAcademicId())->get();
                foreach ($eligible_students as $SingleStudent) {
                    foreach ($eligible_subjects as $subject) {
            ...
            if ($request->student != '') {
                ...
                $grade_chart = SmMarksGrade::select('grade_name', 'gpa', 'percent_from as start', 'percent_upto as end', 'description')->where('active_status', 1)->where('academic_id', YearCheck::getAcademicId())->get()->toArray();
    ...
    {
        try {
            $exams = SmExam::where('active_status', 1)->where('academic_id', YearCheck::getAcademicId())->get();
            $classes = SmClass::where('active_status', 1)->where('academic_id', YearCheck::getAcademicId())->get();
    ...
    public function progressCardReportSearch(Request $request)
    {
        ...
        try {
            $exams = SmExam::where('active_status', 1)->where('class_id', $request->class)->where('section_id', $request->section)->where('academic_id', YearCheck::getAcademicId())->get();
    ...
    {
        try {
            $user_logs = SmUserLog::where('academic_id', YearCheck::getAcademicId())->get();
            if (ApiBaseMethod::checkUrl($request->fullUrl())) {
    ...
    public function studentHomework(Request $request, $id = null)
    {
        ...
        if (ApiBaseMethod::checkUrl($request->fullUrl())) {
            ...
            $subject_list = SmAssignSubject::where([['class_id', $class_id], ['section_id', $student_detail->section_id]])->where('academic_id', YearCheck::getAcademicId())->get();
            ...
            foreach ($subject_list as $subject) {
                ...
                $allList[$subject->subject->subject_name] = DB::table('sm_homeworks')
                    ...
                    ->where('subject_id', $subject->subject_id)->where('sm_homeworks.academic_id', YearCheck::getAcademicId())->get()->toArray();;
            }
            foreach ($allList as $single) {
                foreach ($single as $singleHw) {
                    $std_homework = DB::table('sm_homework_students')
                        ...
                        ->where('sm_homework_students.academic_id', YearCheck::getAcademicId())
                        ->first();
    ...
    public function saas_studentHomework(Request $request,$school_id, $id = null)
    {
        ...
        if (ApiBaseMethod::checkUrl($request->fullUrl())) {
            ...
            $class_id = @$student_detail->class->id;
            $subject_list = SmAssignSubject::where([['class_id', $class_id], ['section_id', @$student_detail->section_id]])->where('academic_id', YearCheck::getAcademicId())->where('school_id',$school_id)->get();
            ...
            foreach ($subject_list as $subject) {
                ...
                $allList[$subject->subject->subject_name] = DB::table('sm_homeworks')
                    ->leftjoin('sm_subjects', 'sm_subjects.id', '=', 'sm_homeworks.subject_id')
                    ->where('class_id', @$student_detail->class_id)->where('section_id', @$student_detail->section_id)
                    ->where('subject_id', $subject->subject_id)->where('sm_homeworks.academic_id', YearCheck::getAcademicId())->where('sm_homeworks.school_id',$school_id)->get()->toArray();;
            }
            foreach ($allList as $single) {
                foreach ($single as $singleHw) {
                    $std_homework = DB::table('sm_homework_students')
            ...
            $homeworkLists = SmHomework::where('class_id', @$student_detail->class_id)->where('section_id', @$student_detail->section_id)
                ->where('academic_id', YearCheck::getAcademicId())->where('school_id',$school_id)->get();
        } else {
            $user = Auth::user();
    ...
    public function studentDashboard(Request $request, $id = null)
    {
        ...
        $driver = SmVehicle::where('sm_vehicles.id', '=', $student_detail->vechile_id)
            ...
            ->first();
        $siblings = SmStudent::where('parent_id', $student_detail->parent_id)->get();
        $fees_assigneds = SmFeesAssign::where('student_id', $student_detail->id)->where('academic_id', YearCheck::getAcademicId())->get();
        $fees_discounts = SmFeesAssignDiscount::where('student_id', $student_detail->id)->where('academic_id', YearCheck::getAcademicId())->get();
        $documents = SmStudentDocument::where('student_staff_id', $student_detail->id)->where('type', 'stu')->where('academic_id', YearCheck::getAcademicId())->get();
        $timelines = SmStudentTimeline::where('staff_student_id', $student_detail->id)->where('type', 'stu')->where('visible_to_student', 1)->where('academic_id', YearCheck::getAcademicId())->get();
        $exams = SmExamSchedule::where('class_id', $student_detail->class_id)->where('section_id', $student_detail->section_id)->where('academic_id', YearCheck::getAcademicId())->get();
        $grades = SmMarksGrade::where('active_status', 1)->where('academic_id', YearCheck::getAcademicId())->get();
    ...
    public function saas_studentDashboard(Request $request,$school_id, $id = null)
    {
        ...
        $driver = SmVehicle::where('sm_vehicles.id', '=', @$student_detail->vechile_id)
            ...
            ->first();
        $siblings = SmStudent::where('parent_id', @$student_detail->parent_id)->where('school_id',$school_id)->get();
        $fees_assigneds = SmFeesAssign::where('student_id', @$student_detail->id)->where('academic_id', YearCheck::getAcademicId())->where('school_id',$school_id)->get();
        $fees_discounts = SmFeesAssignDiscount::where('student_id', @$student_detail->id)->where('academic_id', YearCheck::getAcademicId())->where('school_id',$school_id)->get();
        $documents = SmStudentDocument::where('student_staff_id', @$student_detail->id)->where('type', 'stu')->where('academic_id', YearCheck::getAcademicId())->where('school_id',$school_id)->get();
        $timelines = SmStudentTimeline::where('staff_student_id', @$student_detail->id)->where('type', 'stu')->where('visible_to_student', 1)->where('academic_id', YearCheck::getAcademicId())->where('school_id',$school_id)->get();
        $exams = SmExamSchedule::where('class_id', @$student_detail->class_id)->where('section_id', @$student_detail->section_id)->where('academic_id', YearCheck::getAcademicId())->where('school_id',$school_id)->get();
        $grades = SmMarksGrade::where('active_status', 1)->where('academic_id', YearCheck::getAcademicId())->get();
    ...
    public function studentSubjectApi(Request $request, $id)
    {
        ...
        $assignSubjects = DB::table('sm_assign_subjects')
            ...
            ->where('sm_assign_subjects.academic_id', YearCheck::getAcademicId())->get();
        if (ApiBaseMethod::checkUrl($request->fullUrl())) {
            $data = [];
    ...
    public function saas_studentSubjectApi(Request $request,$school_id, $id)
    {
        ...
        $assignSubjects = DB::table('sm_assign_subjects')
            ...
            ->where('sm_assign_subjects.class_id', '=', @$student->class_id)
            ->where('sm_assign_subjects.section_id', '=', @$student->section_id)
            ->where('sm_assign_subjects.academic_id', YearCheck::getAcademicId())->where('sm_assign_subjects.school_id',$school_id)->get();
        if (ApiBaseMethod::checkUrl($request->fullUrl())) {
            $data = [];
            $data['student_subjects'] = $assignSubjects->toArray();
            return ApiBaseMethod::sendResponse($data, null);
    ...
    public function studentExamScheduleApi(Request $request, $id)
    {
        ...
        if (ApiBaseMethod::checkUrl($request->fullUrl())) {
            ...
            // $assign_subjects = SmAssignSubject::where('class_id', $student_detail->class_id)->where('section_id', $student_detail->section_id)->where('academic_id', YearCheck::getAcademicId())->get();
            ...
            $exam_schedule = DB::table('sm_exam_schedules')
                ...
                ->where('sm_exam_schedules.academic_id', YearCheck::getAcademicId())->get();
    ...
    public function saas_studentExamScheduleApi(Request $request,$school_id, $id)
    {
        ...
        if (ApiBaseMethod::checkUrl($request->fullUrl())) {
            ...
            $exam_schedule = DB::table('sm_exam_schedules')
                ...
                ->where('sm_exam_schedules.section_id', '=', @$student_detail->section_id)
                ->where('sm_exam_schedules.academic_id', YearCheck::getAcademicId())->
    ...
    public function studentTimelineApi(Request $request, $id)
    {
        ...
        if (ApiBaseMethod::checkUrl($request->fullUrl())) {
            ...
            $timelines = DB::table('sm_student_timelines')
                ...
                ->where('sm_student_timelines.academic_id', YearCheck::getAcademicId())->get();
    ...
    public function saas_studentTimelineApi(Request $request,$school_id, $id)
    {
        ...
        if (ApiBaseMethod::checkUrl($request->fullUrl())) {
            ...
            $timelines = DB::table('sm_student_timelines')
                ...
                ->where('sm_student_timelines.academic_id', YearCheck::getAcademicId())->
    ...
    public function teacherClassList(Request $request)
    {
        ...
        if (ApiBaseMethod::checkUrl($request->fullUrl())) {
            ...
            if ($teacher->role_id==1) {
                ...
                ->where('teacher_id', $teacher_id)
                ->where('sm_classes.academic_id', YearCheck::getAcademicId())
                ->get();
            }
    ...
    public function homework_List(Request $request, $id)
    {
        ...
        //     return ApiBaseMethod::sendError('assign subject not found.', null);
        // }
        $i = 0;
        $allList=[];
        $kijanidibo=[];
