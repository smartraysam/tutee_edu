                ...
                <div class="table-responsive">
                                ...
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right">
                                     ...
                                     @if(@$data['bank_info']->active_status == 1 || @$data['cheque_info']->active_status == 1)
                                        ...
                                        <a class="dropdown-item modalLink" data-modal-size="modal-lg" title="{{$fees_assigned->feesGroupMaster->feesGroups->name.': '. $fees_assigned->feesGroupMaster->feesTypes->name}}"  href="{{url('fees-generate-modal-child-view', [$already_add->id])}}" >@lang('lang.view') @lang('lang.bank') @lang('lang.payment')</a>
                                        ...
                                        @if($already_add->approve_status == 0)
                                            ...
                                            <a onclick="deleteId({{$already_add->id}});" class="dropdown-item" href="#" data-toggle="modal" data-target="#deleteStudentModal" data-id="{{$already_add->id}}"  >@lang('lang.delete') @lang('lang.bank') @lang('lang.payment')</a>
                                    ...
                                    @if(!empty($is_paystack))
                                        <form method="POST" action="{{ route('pay-with-paystack') }}" accept-charset="UTF-8" class="form-horizontal" role="form">
                                            ...
                                            <input type="hidden" name="amount" value="{{$fees_assigned->fees_amount * 100}}"> {{-- required in kobo --}}
                                            ...
                                            <input type="hidden" name="key" value="{{ @$paystack_info->gateway_secret_key }}"> {{-- required --}}
                                    ...
                                    @if(!empty($is_stripe))
                                        ...
                                        <a class="dropdown-item modalLink" data-modal-size="modal-lg" title="@lang('lang.pay') @lang('lang.fees') " href="{{url('fees-payment-stripe', [@$fees_assigned->feesGroupMaster->fees_type_id, $student->id, $rest_amount])}}">Pay via Stripe</a>

                                    @endif

                                    {{-- Start Razorpay Checking --}}

                                    @if(App\SmGeneralSettings::isModule('RazorPay') == TRUE)
                                        ...
                                        @endphp
                                            @if(!empty($is_RazorPay))
                                                <form id="rzp-footer-form_{{$count}}" action="{!!url('razorpay/dopayment')!!}" method="POST" style="width: 100%; text-align: center" >
                                                    ...
                                                    <input type="hidden" name="amount" id="amount" value="{{$fees_assigned->fees_amount * 100}}"/>
                                                    ...
                                                    <input type="hidden" name="amount" id="amount" value="{{$fees_assigned->fees_amount}}"/>
                                                    <div class="pay">
                    ...
                    <tfoot>
                        <tr>
                            <th></th>
                            <th></th>
                            <th>@lang('lang.grand') @lang('lang.total') ({{@$currency}})</th>
                            <th></th>
                            <th>{{@$grand_total}}</th>
                            <th></th>
                            <th></th>
