...
$active_style = DB::table('sm_styles')
->where('is_active', 1)
->first();
$styles = DB::table('sm_styles')
->where('school_id', 1)
->get();
$generalSetting = $config = DB::table('sm_general_settings')->first();
$schoolSetting = $school_config = DB::table('sm_general_settings')
->where('school_id', Auth::user()->school_id)
->first();
$system_date_foramt = App\SmDateFormat::find($config->date_format_id);
$dashboard_background = DB::table('sm_background_settings')
->where([['is_default', 1], ['title', 'Dashboard Background']])
->first();
...
<body class="admin" style=" <?php if($active_style->id == 1): ?> <?php echo e($css); ?> <?php else: ?>
    background:<?php echo e($active_style->dashboardbackground); ?> !important; <?php endif; ?> ">
...
<?php /**PATH C:\laragon\www\tutee_edu\resources\views/backEnd/partials/header.blade.php ENDPATH**/ ?>